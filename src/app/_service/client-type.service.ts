...
export class ClientTypeService {
  ...
  get(page:any,params:any) : Observable<ClientType[]> {
    return this._http.post(this.apiUrl+'get'+this.title+'?page='+page,params,this.options)
    .map((res:Response) => <ClientType[]>res.json())
    .catch((error:any) => Observable.throw(error.json().error || 'Server error'));
  }

  add(param:any) : Observable<ClientType[]> {
    console.log(param);
  ...
  update(params:any,id:number) : Observable<ClientType[]> {
  ...
  delete(id:number) : Observable<ClientType[]> {
